...
{
    public static AsyncPipeline<FlatArray<TOut>, TFailure> ForwardParallelValue<TIn, TOut, TFailure>(
        this AsyncPipeline<FlatArray<TIn>, TFailure> pipeline,
        Func<TIn, CancellationToken, ValueTask<Result<TOut, TFailure>>> forwardAsync,
        ...
        where TFailure : struct
    {
        ...
        ValueTask<FlatArray<Result<TOut, TFailure>>> InnerForwardAsync(FlatArray<TIn> input, CancellationToken cancellationToken)
            =>
            input.InnerPipeParallelValueAsync(forwardAsync, option, pipeline.Configuration, cancellationToken);
