...
{
    public static AsyncPipeline<(T1, T2, T3), TFailure> ForwardParallelValue<TIn, T1, T2, T3, TFailure>(
        ...
        where TFailure : struct
    {
        ...
        return pipeline.MapSuccessValue(InnerForwardAsync).Forward(InnerJoinSuccess<TIn, T1, T2, T3, TFailure>);
